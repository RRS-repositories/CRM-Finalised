...
import { Shield, Key, Users, Copy, Eye, EyeOff, Edit, Check, X, AlertTriangle } from 'lucide-react';
...
const AdminPanel: React.FC = () => {
  const { users, updateUserRole, updateUserStatus, currentUser } = useCRM();
                  ...
                  <tr key={user.id} className="hover:bg-slate-50 dark:hover:bg-slate-700">
                    ...
                    <td className="px-6 py-4">
                       <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border capitalize ${
                         user.role === 'Management' ? 'bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 border-purple-100 dark:border-purple-800' : 'bg-gray-50 dark:bg-slate-700 text-gray-700 dark:text-gray-300 border-gray-100 dark:border-slate-600'
                       }`}>
                         {user.role}
                       </span>
                    </td>
