...
const CATEGORY_1_NO_ID_LENDERS = new Set([
    ...
    'BETTER BORROW', 'BIP CREDIT', 'BIP CREDIT CARD', 'CABOT', 'CABOT FINANCIAL', 'CAPITAL ONE', 'CAPQUEST/ ARROW', 'CAPQUEST', 'ARROW', 'CAR CASH POINT', 'CASHFLOAT',
    ...
    'DANSKE BANK', 'DEBENHAMS', 'DOROTHY PERKINS', 'DRAFTY LOANS', 'EVANS CATALOGUE', 'EVERYDAY LENDING', 'EVERY DAY LENDING', 'EVOLUTION LENDING', 'EVOLUTION MONEY',
    'FAIR FINANCE', 'FASHION WORLD', 'FERNOVO', 'FINIO LOANS', 'FINIO/LIKELY LOANS', 'FINTERN/ABOUND', 'FINTERN', 'ABOUND', 'FLUID', 'LIKELY LOANS',
    'FREEMANS CATALOUGE', 'FUND OURSELVES', 'G.L.M. FINANCE', 'GE CAPITAL', 'GRATTAN', 'GUARANTOR MY LOAN',
    'GURRANTOR MY LOAN', 'H&T PAWNBROKERS', 'H&T PAWN BROKERS', 'HALIFAX', 'HERO LOANS', 'HSBC', 'HSBC BANK', 'ICO', 'INTRUM',
    'JD WILLIAMS', 'JUO LOANS', 'KAYS', 'KLARNA', 'KOYO LOANS', 'LANTERN', 'LENDABLE', 'LENDING WORKS', 'LIFE STYLE LOANS',
    'LINK FINANCIAL', 'LINK', 'LITTLE LOANS', 'LITTLEWOODS / GREAT UNIVERSAL', 'LITTLEWOODS', 'GREAT UNIVERSAL', 'LIVE LEND',
    'LLOYDS BANK / BANK OF SCOTLAND / MBNA', 'LLOYDS BANK', 'LLOYDS OVERDRAFT', 'LOAN4YOU / NA', 'LOAN4YOU', 'LOANS 2 GO', 'LOANS2GO',
    'LOGBOOK LENDING', 'LOGBOOK MONEY', 'LOWELL', 'LUMA', 'MARKS & SPENCERS', 'MBNA',
    'METRO BANK/RATE SETTER', 'METRO BANK', 'RATE SETTER', 'MONEY BARN', 'MONEY BOAT', 'MONEYBOAT', 'MONEY WAY', 'MONTHLY ADVANCE LOANS', 'MONZO',
    'MUIRHEAD FINANCE', 'MUTUAL FINANCE', 'MY COMMUNITY BANK', 'MY COMMUNITY FINANCE', 'MY FINANCE CLUB',
    'MY KREDIT', 'NATIONWIDE', 'NATWEST / RBS OVERDRAFT', 'RBS OVERDRAFT', 'NATWEST BANK / ROYAL BANK OF SCOTLAND', 'ROYAL BANK OF SCOTLAND',
    'NEWDAY / OPUS / MARBLES / FLUID / BURTONS', 'NEWDAY', 'OPUS', 'MARBLES', 'BURTONS', 'NEXT', 'NORWICH TRUST', 'NOVUNA', 'NOVUNA FINANCE', 'OCEAN FINANCE',
    'ONDAL', 'ONDAL FINANCE', 'ONE PLUS ONE LOANS', 'ONMO', 'OPLO', 'OPOLO', 'PAYPAL', 'PEACHY LOANS', 'PERCH GROUP',
    'PLATA FINANCE', 'PLEND', 'PM LOANS', 'POLAR FINANCE', 'POST OFFICE', 'POST OFFICE MONEY', 'PRA', 'PRA GROUP', 'PROGRESSIVE MONEY',
    'PROVIDENT', 'PSA FINANCE', 'QUID MARKET', 'REEVO', "SAINSBURY'S BANK", 'SALAD MONEY', 'SALARY FINANCE',
    'SAVVY LOANS', 'SAVVY LOANS.CO.UK', 'SHAWBROOK BANK', 'SHORT TERM FINANCE', 'SIMPLY BE', 'SKYLINE DIRECT', 'SNAP FINANCE', 'STUDIO',
    'SUCO', 'SWIFT LOANS', 'TANDEM', 'TAPPILY', 'TESCO BANK', 'THINKMONEY', 'TM ADVANCES', 'TRANSUNION',
    'TSB', 'ULSTER BANK', 'UPDRAFT', 'VERY / SHOP DIRECT / G UNIVERSAL', 'SHOP DIRECT', 'VERY CATALOGUE',
    'VIRGIN/ CLYDESDALE/ YORKSHIRE BANK', 'VIRGIN', 'CLYDESDALE', 'YORKSHIRE BANK', 'WAGE DAY ADVANCES', 'WAGE STREAM', 'WAVE', 'ZABLE',
    'ZEMPLER BANK / CASHPLUS', 'ZEMPLER BANK', 'CASHPLUS', 'ZOPA', 'TEST'
]);
...
const LENDER_ALIASES = {
    ...
    // H&T: ampersand gets stripped differently (HT vs H_T) in S3 paths
    'H&T PAWNBROKERS': ['H&T PAWNBROKERS', 'HT PAWNBROKERS', 'HT PAWN BROKERS', 'H T PAWNBROKERS'],
    'H&T PAWN BROKERS': ['H&T PAWN BROKERS', 'HT PAWN BROKERS', 'HT PAWNBROKERS', 'H T PAWN BROKERS'],
};
...
async function gatherDocumentsForCase(contactId, lenderName, folderName, caseId, firstName, lastName) {
    ...
    for (const folder of allFolders) {
        ...
        // Fallback: check Documents/Letter_of_Authority/ and Documents/LOA/ (backend uploads LOAs here)
        if (!documents.loa) {
            documents.loa = await findFileInS3Folder(`${folder}/Documents/Letter_of_Authority/`, lenderName, 'LOA', refSpec);
        }
        if (!documents.loa) {
            documents.loa = await findFileInS3Folder(`${folder}/Documents/LOA/`, lenderName, 'LOA', refSpec);
        }
        ...
        <tr>
            <td align="center" style="padding: 40px 20px;">
                <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="background-color: #ffffff; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                    ...
                    // DSAR Overdue: always send directly (no drafts)
                    ...
                    try {
                        ...
                        const info = await lenderEmailTransporter.sendMail(mailOptions);
                        console.log(`[Worker] ✅ DSAR Overdue email SENT to ${lenderName} (${lenderEmail}). MessageId: ${info.messageId}`);
                        ...
                        await pool.query(
                            ...
                            [record.contact_id, record.case_id, `[${successTimestamp}] DSAR Overdue email sent to ${lenderName} (${lenderEmail})`]
                        );
                    } catch (sendErr) {
                        console.error(`[Worker] ❌ Failed to send DSAR overdue email for Case ${record.case_id}:`, sendErr.message);
                        ...
                        await pool.query(
                            ...
                            [record.contact_id, record.case_id, `[${failTimestamp}] Failed to send DSAR overdue email to ${lenderName}: ${sendErr.message}`]
                        );
                    }
                } else {
